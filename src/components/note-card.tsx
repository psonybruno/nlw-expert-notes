...
export function NoteCard({ note, onNoteDeleted }: NoteCardProps) {
  return (
    <Dialog.Root>
      ...
      <span className="text-sm font-medium text-slate-300">{formatDistanceToNow(note.date.toISOString(), { locale: ptBR, addSuffix: true})}</span>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className='fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2  md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none'>
          ...
          <button 
            ...
            className='w-full bg-slate-800 py-4 text-center text-sm text-slate-300 outline-none font-medium group'>Deseja <span className='text-red-400 group-hover:underline '>apagar essa nota</span>?</button>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
  )
